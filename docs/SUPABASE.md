...
> **잘리지 않게 복사하세요.** 이 파일은 674줄입니다. 화면에 보이는 부분만 드래그하면 앞부분만 복사됩니다.
> GitHub 파일 화면 오른쪽 위의 **복사 아이콘**을 쓰면 전체가 한 번에 복사됩니다.
> 붙여넣은 뒤 편집기 맨 아래 줄 번호가 **674** 인지 확인하세요.

끝까지 실행되면 `설치 완료: 표 21개가 준비되었습니다` 라는 메시지가 나옵니다.
이 메시지가 없으면 파일이 잘린 것이니 다시 복사해 주세요. 여러 번 실행해도 안전합니다. 이 파일 하나에 아래 세 가지가 순서대로 들어 있습니다.
...
두 사람은 **각자의 이메일과 이름으로 따로 가입**합니다. 계정은 둘이지만 보는 공간은 하나입니다.

1. 받은 초대 링크를 엽니다.
2. 회원가입을 합니다. (이메일 인증 메일이 오면 링크를 누릅니다)
3. 초대 코드가 미리 채워진 화면에서 **참여하기** 를 누릅니다.

초대 코드는 링크를 한 번 연 기기에 기억해 둡니다. 이메일 인증을 하고 돌아와도 다시 물어보지 않습니다.
코드를 잃어버렸다면 신랑의 **설정 › 계정** 화면에 8자리 코드가 늘 떠 있습니다.
...
### 로그인 후 "Could not find the table ... in the schema cache" 가 떠요

로그인은 됐지만 **표가 아직 만들어지지 않은** 상태입니다. 원인은 보통 셋 중 하나입니다.

1. **다른 프로젝트에서 실행했다.** 앱 화면에 "지금 연결된 곳" 으로 나오는 주소와 같은 프로젝트인지 확인하세요.
2. **파일이 잘려서 앞부분만 실행됐다.** 674줄 전부인지 확인하고 다시 Run 하세요.
   앞부분만 실행된 상태에서 다시 전체를 Run 해도 안전하게 이어서 만들어집니다.
3. **표는 있는데 캐시가 옛날 상태다.** SQL Editor 에서 `notify pgrst, 'reload schema';` 를 한 번 실행하세요.

어느 쪽인지 알고 싶으면 SQL Editor 에서 이걸 실행해 보세요. 21이 나오면 설치가 끝난 것입니다.
