...
export function TaskRow({ task, onOpen }: { task: Task; onOpen: (id: string) => void }) {
  ...
  return (
    <motion.li layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-3 px-4 py-3 min-h-[3.5rem] hover:bg-surface-2">
      ...
      <button type="button" onClick={() => onOpen(task.id)} className="min-w-0 flex-1 text-left">
        <span className="flex items-center gap-2">
          ...
          <span className={cn("truncate text-[0.9375rem] font-medium", done ? "text-fg-3 line-through decoration-fg-3/50" : "text-fg")}>{task.title}</span>
