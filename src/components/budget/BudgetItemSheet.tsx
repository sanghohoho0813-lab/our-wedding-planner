    ...
    >
      <div className="space-y-5">
        ...
        {values.actual_amount > 0 && values.estimated_amount > 0 && (
          <div
            className={cn(
              "flex items-center justify-between rounded-[12px] px-3.5 py-2.5 text-[0.875rem]",
        ...
        {isEdit && (
          <div className="space-y-3 rounded-[16px] border border-line bg-surface-2/60 p-4">
            <div className="flex items-center justify-between">
              <h3 className="text-[0.9375rem] font-semibold text-fg">결제</h3>
              <div className="text-right text-[0.75rem] text-fg-3">
              ...
              <ul className="divide-y divide-line rounded-[12px] border border-line bg-surface">
                {itemPayments.map((p) => (
                  <li key={p.id} className="flex items-center gap-3 px-3 py-2.5">
                    <CheckCircle
                      checked={p.paid}
                      onChange={(v) => patch("payments", p.id, { paid: v, paid_at: v ? todayISO() : null })}
                      label={p.paid ? "결제 완료 취소" : "결제 완료로 표시"}
                    />
                    <div className="min-w-0 flex-1">
                      <div className={cn("truncate text-[0.875rem] font-medium", p.paid && "text-fg-3")}>{p.title}</div>
                    ...
                    <span className="tabular text-[0.875rem] font-semibold">{formatKRW(p.amount)}</span>
                    <button
                      type="button"
                      aria-label="결제 삭제"
                      onClick={() => remove("payments", p.id)}
                      className="inline-flex size-9 items-center justify-center rounded-full text-fg-3 hover:bg-surface-2 hover:text-danger"
                    >
                      <Trash2 className="size-4" />
                    </button>
                  </li>
            ...
            {addingPayment ? (
              <div className="space-y-3 rounded-[12px] border border-line bg-surface p-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="flex gap-1.5">
                    <Chip size="sm" active={!pPaid} onClick={() => setPPaid(false)} className="flex-1 justify-center h-11">
                      결제 예정
                    </Chip>
                    <Chip size="sm" active={pPaid} onClick={() => setPPaid(true)} className="flex-1 justify-center h-11" tone="success">
                      결제 완료
                    </Chip>
                  </div>
                </div>
                {unpaid > 0 && pAmount !== unpaid && (
                  <button type="button" onClick={() => setPAmount(unpaid)} className="text-[0.8125rem] text-accent-text hover:underline">
              ...
              <Button variant="outline" full onClick={() => { setAddingPayment(true); setPAmount(unpaid); }}>
