...
export function SaveIndicator({ className }: { className?: string }) {
  ...
  if (pending > 0)
    return (
      <span className={cn("inline-flex items-center gap-1 text-[0.75rem] text-fg-3", className)} aria-live="polite">
        <Loader2 className="size-3.5 animate-spin" /> 저장 중…
      </span>
    );
  if (showSaved)
    return (
      <span className={cn("inline-flex items-center gap-1 text-[0.75rem] text-success", className)} aria-live="polite">
