...
import { daysUntil, formatDDay, formatKoreanDate, todayISO } from "@/lib/date";
  ...
  return (
    <section className="grid gap-3 lg:grid-cols-[1.15fr_1fr_0.85fr]">
      <div className="relative order-2 hidden overflow-hidden rounded-[22px] border border-line hero-gradient p-6 lg:order-1 lg:block">
        ...
        <p className="font-hand text-[2rem] leading-tight text-fg">
          좋은 날,
          <br />
          우리의 이야기
        </p>
        <p className="mt-4 max-w-[16rem] text-[0.875rem] leading-relaxed text-fg-2">
          하나하나 준비하는 모든 순간이
          <br />
          소중한 추억이 될 거예요.
        </p>
        {names && <p className="mt-6 text-[0.8125rem] font-medium text-accent-text">{names}</p>}
      </div>
      ...
      <div className="relative order-1 overflow-hidden rounded-[22px] border border-line bg-surface px-6 py-7 text-center shadow-[var(--shadow-sm)] lg:order-2">
        ...
        <p className="text-[0.9375rem] font-medium text-fg-2">우리 결혼식까지</p>
        <p className="mt-1 font-script text-[4rem] leading-none text-accent-text sm:text-[4.5rem]" aria-label={`디데이 ${dday}`}>
          {dday}
        </p>
        <p className="mt-2 text-[0.9375rem] font-medium text-fg">{formatKoreanDate(wedding.wedding_date)}</p>
        <div className="mt-4 inline-flex items-center gap-2 rounded-full bg-surface-2 px-3.5 py-1.5 text-[0.8125rem]">
          <Sun className="size-4 text-warning" />
          <LiveClock />
        </div>
        <p className="mt-4 text-[0.75rem] text-fg-3">“지금 이 순간도, 우리의 결혼을 만들어가는 날이에요.”</p>
      </div>
      ...
      <div className="relative order-3 hidden overflow-hidden rounded-[22px] border border-line bg-accent-softer p-6 lg:block">
        ...
        <p className="relative text-[1.125rem] font-semibold leading-relaxed text-fg">
          좋은 하루예요!
          <br />
          오늘도
          <br />
        ...
        <p className="relative mt-4 text-[0.8125rem] text-fg-3">{days > 0 ? `${days}일 남았어요. 하나씩 차근차근.` : days === 0 ? "드디어 오늘! 축하해요." : "결혼을 축하해요."}</p>
      </div>
    </section>
  );
}
